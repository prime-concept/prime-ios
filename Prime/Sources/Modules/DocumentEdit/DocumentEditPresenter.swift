...
extension Notification.Name {
	static let profileDocumentsChanged = Notification.Name("Profile.Documents.Changed")
}

protocol DocumentEditPresenterProtocol: AnyObject {
    func didLoad()
    func saveForm()
    func deleteForm()
...
final class DocumentEditPresenter: DocumentEditPresenterProtocol {
    ...
    init(
        ...
        documentsService: DocumentsServiceProtocol,
    ...
    func didLoad() {
        ...
        self.filesService
                ...
                when(fulfilled: imagePromises)
                    ...
                    .catch { error in
						AnalyticsReportingService
							.shared.log(
								name: "[ERROR] \(Swift.type(of: self)) attachments fetch failed",
			...
			.catch { error in
				AnalyticsReportingService
					.shared.log(
						name: "[ERROR] Document attachemnts list fetch failed",
						parameters: error.asDictionary.appending("documentId", documentId)
					)
				self.showCommonError()
				self.viewController?.hideActivity()
    ...
    func saveForm() {
        ...
        queue.promise { () -> Promise<Document> in
            if self.document.id != nil {
                return self.documentsService.update(document: self.document)
            }

            return self.documentsService.create(document: self.document)
        }
        .then(on: queue) { createdVisa -> Promise<Void> in
            ...
            if shouldAttachVisaToPassport, let passportID = self.newVisaRelatedPassportID {
                return self.documentsService.attach(visa: createdVisa, toPassportWithID: passportID)
            }
            ...
            Promise<Void>() { seal in
                ...
                when(fulfilled: self.attachmentPromises)
                    ...
                    .catch {
						AnalyticsReportingService
							.shared.log(
								name: "[ERROR] \(Swift.type(of: self)) attachments upload failed",
        ...
        .done { [weak self] _ in
			NotificationCenter.default.post(name: .profileDocumentsChanged, object: nil)
            delay(0.5) {
    ...
    func deleteForm() {
        ...
        DispatchQueue.main.promise {
            self.documentsService.delete(with: id)
        }
        .done { [weak self] _ in
