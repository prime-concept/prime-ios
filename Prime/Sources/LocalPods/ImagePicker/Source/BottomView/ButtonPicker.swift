...
class ButtonPicker: UIButton {
  ...
  public init(imagePickerConfiguration: ImagePickerConfiguration? = nil) {
    if let imagePickerConfiguration = imagePickerConfiguration {
      self.imagePickerConfiguration = imagePickerConfiguration
    }
  ...
  func setupButton() {
	  backgroundColor = Palette.shared.gray5.rawValue
  ...
  @objc func pickerButtonDidPress(_ button: UIButton) {
    backgroundColor = Palette.shared.gray5.rawValue
    numberLabel.textColorThemed = Palette.shared.black
    numberLabel.sizeToFit()
    delegate?.buttonDidPress()
  }

  @objc func pickerButtonDidHighlight(_ button: UIButton) {
    numberLabel.textColorThemed = Palette.shared.gray5
