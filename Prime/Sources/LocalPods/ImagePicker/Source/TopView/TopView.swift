...
open class TopView: UIView {
  ...
  let flashButtonTitles = [
    "camera.flash.auto".localized,
    "camera.flash.on".localized,
    "camera.flash.off".localized
  ]

  open lazy var flashButton: UIButton = { [unowned self] in
    let button = UIButton()
    button.setImage(AssetManager.getImage("AUTO"), for: UIControl.State())
    button.setTitle("camera.flash.auto".localized, for: UIControl.State())
    button.titleEdgeInsets = UIEdgeInsets(top: 0, left: 4, bottom: 0, right: 0)
	button.setTitleColor(Palette.shared.gray5.rawValue, for: UIControl.State())
    button.setTitleColor(Palette.shared.gray5.rawValue, for: .highlighted)
  ...
  func configure() {
    ...
    for button in buttons {
      button.layer.shadowColorThemed = Palette.shared.black
